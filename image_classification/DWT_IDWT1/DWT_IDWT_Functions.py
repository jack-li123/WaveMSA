...
class DWTFunction_1D(Function):
    ...
    def backward(ctx, grad_L, grad_H):
        ...
        matrix_L = matrix_L.to(grad_L.dtype)
        matrix_H = matrix_H.to(grad_L.dtype)
...
class IDWTFunction_1D(Function):
    ...
    def forward(ctx, input_L, input_H, matrix_L, matrix_H):
        ...
        matrix_L = matrix_L.to(device)
        matrix_H = matrix_H.to(device)
    ...
    def backward(ctx, grad_output):
        matrix_L, matrix_H = ctx.saved_variables
        matrix_L = matrix_L.to(grad_output.dtype)
        matrix_H = matrix_H.to(grad_output.dtype)
...
class IDWTFunction_2D(Function):
    ...
    def forward(ctx, input_LL, input_LH, input_HL, input_HH,
        ...
        input_LL = input_LL.to(device)
        input_LH = input_LH.to(device)
        input_HL = input_HL.to(device)
        input_HH = input_HH.to(device)
